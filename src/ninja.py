...
class WebsiteVulnerabilityScanner:
    ...
    def test_sql_injection(self, url, test_number, proxies=None):
        """
    ...
    def test_xss(self, url, test_number, proxies=None):
        """
    ...
    def test_csrf(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "csrf_token" not in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_directory_traversal(self, url, test_number, proxies=None):
        response = self.session.get(url + "../../../../etc/passwd", proxies=proxies)
        if "root:" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_remote_code_execution(self, url, test_number, proxies=None):
        payload = "<?php echo shell_exec('id'); ?>"
        response = self.session.post(url, data={'code': payload}, proxies=proxies)
        if "uid=" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_insecure_file_upload(self, url, test_number, proxies=None):
    ...
    def test_insecure_crypto(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "md5(" in response.text or "SHA1(" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_insecure_deserialization(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "pickle" in response.text or "marshmallow" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_insecure_redirect(self, url, test_number, proxies=None):
    ...
    def test_file_inclusion(self, url, test_number, proxies=None):
        response = self.session.get(url + "?file=../../../../etc/passwd", proxies=proxies)
        if "root:" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_os_command_injection(self, url, test_number, proxies=None):
        payload = "127.0.0.1; ls -la"
        response = self.session.get(url + "?ip=" + payload, proxies=proxies)
        if "passwd" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_ssti(self, url, test_number, proxies=None):
        payload = "{{ 7*'7' }}"
        response = self.session.get(url + "?name=" + payload, proxies=proxies)
        if "7777777" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_ssr(self, url, test_number, proxies=None):
        response = self.session.get(url + "/?url=http://localhost:8080", proxies=proxies)
        if "localhost" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_mass_assignment(self, url, test_number, proxies=None):
        response = self.session.post(url, data={'admin': 'true'}, proxies=proxies)
        if "Welcome admin" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_xpath_injection(self, url, test_number, proxies=None):
        payload = "' or 1=1 or ''='"
        response = self.session.post(url, data={'username': payload, 'password': 'password'}, proxies=proxies)
        if "Welcome admin" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_lfi(self, url, test_number, proxies=None):
        response = self.session.get(url + "?file=../../../../etc/passwd", proxies=proxies)
        if "root:" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_clickjacking(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "X-Frame-Options" not in response.headers or "DENY" not in response.headers["X-Frame-Options"]:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_insecure_cookies(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "HttpOnly" not in response.cookies.get_dict():
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_insecure_login_page(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "login" in response.url.lower() or "signin" in response.url.lower():
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_misconfigured_security_headers(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "Content-Security-Policy" not in response.headers or "X-XSS-Protection" not in response.headers:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_weak_cryptography(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
        if "md5(" in response.text or "SHA1(" in response.text or "base64" in response.text:
            self.vulnerable_links.setdefault(url, set()).add(test_number)

    def test_session_fixation(self, url, test_number, proxies=None):
        response = self.session.get(url, proxies=proxies)
